...
jQuery(document).ready(function ($) {
	...
	$("#ui-datepicker-div").wrap('<div class="adio_element" />');
	 ...
	 */
	$('.adio_upload_file').change(function () {
		formfield = $(this).attr('name');
		$('#' + formfield + '_id').val("");
	});

	$('.adio_upload_button').live('click', function () {
		var buttonLabel;
		formfield = $(this).prev('input').attr('name');
		buttonLabel = 'Use as ' + $('label[for=' + formfield + ']').text();
		tb_show('', 'media-upload.php?post_id=' + $('#post_ID').val() + '&type=file&adio_force_send=true&adio_send_label=' + buttonLabel + '&TB_iframe=true');
		return false;
	});

	$('.adio_remove_file_button').live('click', function () {
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			if (itemurl.match(image)) {
				uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="adio_remove_file_button" rel="' + formfield + '">Remove Image</a></div>';
			} else {
				// No output preview if it's not an image
				// Standard generic output if it's not an image.
				html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
				uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="adio_remove_file_button" rel="' + formfield + '">Remove</a></div>';
			}

			$('#' + formfield).val(itemurl);
			$('#' + formfield + '_id').val(itemid);
			$('#' + formfield).siblings('.adio_upload_status').slideDown().html(uploadStatus);
			tb_remove();
